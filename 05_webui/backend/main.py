...
# Ensure the backend directory is in sys.path when running from workspace root
...
MANIFEST_PATH = os.path.join(WORKSPACE_DIR, "01_preprocessing", "data", "rag_kb_manifest.json")
...
def get_service_details(sno: str, lang: str = "en"):
    ...
    def get_actual_filepath(relative_path: str) -> str:
        if relative_path.startswith("data/"):
            return os.path.join(WORKSPACE_DIR, "01_preprocessing", relative_path)
        return os.path.join(WORKSPACE_DIR, relative_path)


    filepath = get_actual_filepath(rel_path)
    if not os.path.exists(filepath):
        # Fallback to English profile
        rel_path_en = target_service.get("path_en")
        if rel_path_en:
            filepath = get_actual_filepath(rel_path_en)

    if not os.path.exists(filepath):
        raise HTTPException(status_code=404, detail=f"Profile JSON details not found at {filepath}.")
...
async def synthesize_final_response(
    ...
    if query_lang == "en":
        ...
        lang_instruction = (
            ...
            "- Do NOT write in English and do NOT use Roman alphabet/Latin characters (a-z, A-Z) anywhere in the response.\n"
            "- Every single word must be written in Devanagari script.\n"
            "- If the reference context or system rules contain English terms (such as 'affidavit', 'SC/ST certificate', 'mandatory', 'optional', 'spouse', 'domicile certificate', 'criteria', 'All India Services', 'CG Cadre', 'PSU employee', 'Criteria One', 'Criteria Two', 'Criteria Three'), you MUST translate them to their proper Hindi Devanagari equivalents (e.g. 'शपथ पत्र', 'एससी/एसटी प्रमाणपत्र', 'अनिवार्य', 'वैकल्पिक', 'जीवनसाथी', 'मूल निवासी प्रमाण पत्र', 'मानदंड', 'अखिल भारतीय सेवा', 'छग कैडर', 'पीएसयू कर्मचारी', 'पहला मानदंड', 'दूसरा मानदंड', 'तीसरा मानदंड') in your final output. Do NOT write these terms in Latin/English characters (a-z, A-Z) and do NOT leave them untranslated."
        )
...
CRITICAL: Never mention 'RAG', 'English Source', 'Hindi Source', 'context', the synthesis process, internal identifiers (like 'service_id', 'service ID 4', or 'Service ID: 7'), or database serial numbers (like 'sno', 'serial number 1', etc.). Always refer to services by their official user-facing names (e.g. 'SC/ST Caste Certificate', 'Domicile Certificate') and refer only to the official Sewa Setu portal.
...
"""
    ...
    # Check for Devanagari script characters (ignoring Devanagari punctuation like danda '।' / '॥' and digits '०-९')
    devanagari_regex = r'[\u0900-\u0963\u0970-\u097f]'
    
    if query_lang == "hinglish" and re.search(devanagari_regex, final_reply):
        ...
        try:
            ...
            if romanized and not re.search(devanagari_regex, romanized):
    ...
    # Determine final script language to prevent mixed script link suffix
    output_lang = query_lang
    if re.search(devanagari_regex, final_reply):
        output_lang = "hi"
