...
1. **Stage 1 (Semantic Retrieval):** The system calls `retrieve_candidate_services` which uses `multilingual-e5-large` to calculate the cosine similarity between the user's query and the manifest descriptions in `data/rag_kb_manifest.json` for all services. It returns the **Top 3** candidate services.
